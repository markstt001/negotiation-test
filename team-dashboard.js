...
function renderTeamDashboard(team) {
    ...
    container.innerHTML = `
        ...
        <div style="background: linear-gradient(135deg, #0f2027, #203a43); padding: 20px; border-bottom: 2px solid #d4af37; display: flex; align-items: center; justify-content: space-between;">
            ...
            <button onclick="copyInviteCode('${team.inviteCode || ''}')" style="background: rgba(212,175,55,0.2); border: 1px solid #d4af37; color: #d4af37; padding: 8px 16px; border-radius: 980px; font-size: 12px; cursor: pointer;">
                📋 复制
            </button>
        </div>
        ...
        <!-- 概览 Tab -->
        <div id="teamTab-overview" class="team-tab-content" style="padding-bottom: 40px;">
            ${renderOverviewTab(team, members, memberCount, completedCount, pendingCount, distribution, teamType)}
        </div>
        
        <!-- 分析 Tab -->
        <div id="teamTab-analysis" class="team-tab-content" style="display: none; padding-bottom: 40px;">
            ${renderAnalysisTab(dimensions, members)}
        </div>
        
        <!-- 对比 Tab -->
        <div id="teamTab-compare" class="team-tab-content" style="display: none; padding-bottom: 40px;">
            ${renderCompareTab(members)}
        </div>
        
        <!-- 报告 Tab -->
        <div id="teamTab-report" class="team-tab-content" style="display: none; padding-bottom: 40px;">
            ${renderReportTab(members)}
        </div>
    `;
}
...
function analyzeTeamType(dimensions, totalMembers) {
    ...
    var ratios = {
        A: dimensions.A / dimTotal,
        I: dimensions.I / dimTotal,
        R: dimensions.R / dimTotal,
        T: dimensions.T / dimTotal,
        C: dimensions.C / dimTotal,
        B: dimensions.B / dimTotal,
        D: dimensions.D / dimTotal,
        P: dimensions.P / dimTotal
    };
    ...
    if (maxRatio - minRatio < 0.15) {
        return {
            name: '⚖️ 均衡型团队',
            desc: '各维度分布均匀，能力全面，适应性强',
            type: 'balanced'
        };
    }
    
    if (ratios.A > 0.6) {
        return { name: '🧠 分析型团队', desc: '擅长数据分析和逻辑推理，决策谨慎', type: 'thinking' };
    }
    if (ratios.I > 0.6) {
        return { name: '💡 直觉型团队', desc: '擅长创新洞察和机会识别，思维活跃', type: 'thinking' };
    }
    if (ratios.R > 0.6) {
        return { name: '🤝 关系型团队', desc: '擅长关系维护和协调沟通，重视和谐', type: 'orientation' };
    }
    if (ratios.T > 0.6) {
        return { name: '🎯 任务型团队', desc: '擅长目标达成和高效执行，结果导向', type: 'orientation' };
    }
    if (ratios.C > 0.6) {
        return { name: '⚔️ 竞争型团队', desc: '擅长争取利益和谈判博弈，不甘示弱', type: 'approach' };
    }
    if (ratios.B > 0.6) {
        return { name: '🤲 合作型团队', desc: '擅长建立长期合作关系，寻求共赢', type: 'approach' };
    }
    if (ratios.D > 0.6) {
        return { name: '🛡️ 防御型团队', desc: '擅长风险控制和谨慎决策，保守稳健', type: 'strategy' };
    }
    if (ratios.P > 0.6) {
        return { name: '🚀 开拓型团队', desc: '擅长创新尝试和快速行动，敢于冒险', type: 'strategy' };
    }
    
    return {
        name: '🔀 互补型团队',
        desc: '成员风格多样，优势互补，协作潜力大',
        type: 'complementary'
    };
}
...
function renderStyleDistributionChart(distribution, total) {
    ...
    if (entries.length === 0) {
        return '<div style="text-align: center; color: #86868b; padding: 20px;">暂无数据</div>';
    }
...
function generateTeamAdvantages(dimensions) {
    ...
    if (dimensions.A >= dimensions.I && dimensions.A > 0) {
        advantages.push({ title: "数据分析能力强", desc: `团队中 ${dimensions.A} 人是分析型，决策时有数据支撑，不易冲动` });
    } else if (dimensions.I > 0) {
        advantages.push({ title: "创新洞察能力强", desc: `团队中 ${dimensions.I} 人是直觉型，善于发现机会和创新` });
    }
    
    if (dimensions.R >= dimensions.T && dimensions.R > 0) {
        advantages.push({ title: "关系维护能力好", desc: `团队中 ${dimensions.R} 人是关系导向，善于维护供应商关系和内部协调` });
    } else if (dimensions.T > 0) {
        advantages.push({ title: "任务执行能力强", desc: `团队中 ${dimensions.T} 人是任务导向，目标明确，执行力强` });
    }
    
    if (dimensions.C >= dimensions.B && dimensions.C > 0) {
        advantages.push({ title: "竞争意识强", desc: `团队中 ${dimensions.C} 人是竞争型，在谈判中善于争取最大利益` });
    } else if (dimensions.B > 0) {
        advantages.push({ title: "合作共赢意识好", desc: `团队中 ${dimensions.B} 人是合作型，善于建立长期合作关系` });
    }
    
    if (dimensions.D >= dimensions.P && dimensions.D > 0) {
        advantages.push({ title: "风险控制能力强", desc: `团队中 ${dimensions.D} 人是防御型，谨慎保守，善于识别和规避风险` });
    } else if (dimensions.P > 0) {
        advantages.push({ title: "开拓创新能力好", desc: `团队中 ${dimensions.P} 人是开拓型，敢于尝试新方法和新机会` });
    }
...
function generateTeamRisks(dimensions) {
    ...
    if (dimensions.A === 0 && dimensions.I === 0) {
        risks.push({ title: "缺少数据分析者", desc: "团队中分析型成员较少", risk: "容易凭感觉或经验决策，可能忽略关键数据" });
    } else if (dimensions.A < 2 && dimensions.I < 2) {
        risks.push({ title: "数据分析能力弱", desc: "团队中分析型成员不足", risk: "决策时可能缺乏充分的数据支撑" });
    }
    
    if (dimensions.R === 0 && dimensions.T === 0) {
        risks.push({ title: "缺少关系维护者", desc: "团队中关系导向成员较少", risk: "容易把关系搞僵，影响长期合作" });
    } else if (dimensions.R < 2 && dimensions.T < 2) {
        risks.push({ title: "关系维护能力弱", desc: "团队中关系型成员不足", risk: "可能忽略供应商关系和内部协调" });
    }
    
    if (dimensions.C === 0 && dimensions.B === 0) {
        risks.push({ title: "缺少竞争意识", desc: "团队中竞争型成员较少", risk: "在谈判中可能过于妥协，容易吃亏" });
    } else if (dimensions.C < 2 && dimensions.B < 2) {
        risks.push({ title: "竞争意识不足", desc: "团队中竞争型成员不足", risk: "可能无法争取最大利益" });
    }
    
    if (dimensions.D === 0 && dimensions.P === 0) {
        risks.push({ title: "缺少风险控制者", desc: "团队中防御型成员较少", risk: "可能过于冒进，容易踩坑" });
    } else if (dimensions.D < 2 && dimensions.P < 2) {
        risks.push({ title: "风险控制能力弱", desc: "团队中防御型成员不足", risk: "可能忽略潜在风险" });
    }
    ...
    return risks.length > 0 ? risks : [{ title: "风险较低", desc: "团队配置合理，无明显风险点", risk: "继续保持学习和提升" }];
            ...
            <div class="card" style="background: rgba(32,58,67,0.7); border-radius: 16px; overflow: hidden; border: 1px solid rgba(58,106,122,0.3);">
                ...
                <div style="padding: 0;">
                    ${members.length > 0 ? members.map((m, idx) => `
                        <div style="display: flex; align-items: center; padding: 16px; ${idx < members.length - 1 ? 'border-bottom: 1px solid rgba(58,106,122,0.3)' : ''}">
                            ...
                            <div style="text-align: right; flex-shrink: 0;">
                                ${(m.completed || (m.code && m.code.length === 4))
                                    ? '<span style="font-size: 11px; color: #10b981; background: rgba(16,185,129,0.2); padding: 4px 10px; border-radius: 980px;">✅ 已完成</span>'
                                    : '<span style="font-size: 11px; color: #f59e0b; background: rgba(245,158,11,0.2); padding: 4px 10px; border-radius: 980px;">⏳ 待测试</span>'
                                }
                            </div>
                        </div>
                    `).join('') : '<div style="padding: 20px; text-align: center; color: #86868b;">暂无成员</div>'}
                </div>
            </div>
        </div>
        ...
        <div style="margin: 16px; padding: 0 4px;">
            <button onclick="addTeamMember()" style="width: 100%; background: rgba(44,83,100,0.6); color: #fff; border: 2px solid #3a6a7a; padding: 14px; border-radius: 12px; font-size: 14px; font-weight: 600; cursor: pointer; margin-bottom: 12px;">
                ➕ 添加成员
            </button>
            <button onclick="inviteTeamMembers()" style="width: 100%; background: linear-gradient(135deg, #2c5364, #d4af37); color: #0f2027; border: none; padding: 14px; border-radius: 12px; font-size: 14px; font-weight: 600; cursor: pointer;">
                📤 邀请成员
            </button>
        </div>
    `;
...
function renderAnalysisTab(dimensions, members) {
    ...
    if (totalMembers === 0) {
        return '<div style="padding: 40px; text-align: center; color: #86868b;">暂无足够数据进行分析</div>';
    }
    ...
    return `
                        ...
                        <div style="margin-bottom: 20px;">
                            ...
                            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px;">
                                ${dim.items.map(item => {
                                    ...
                                    return `
                                        <div style="background: ${isMajority ? 'rgba(212,175,55,0.15)' : 'rgba(44,83,100,0.3)'}; padding: 12px; border-radius: 8px; border: ${isMajority ? '1px solid #d4af37' : 'none'};">
        ...
        <div style="margin: 16px;">
            <div class="card" style="background: rgba(32,58,67,0.7); border-radius: 16px; overflow: hidden; border: 1px solid rgba(58,106,122,0.3);">
                ...
                <div style="padding: 20px;">
                    ${generateTeamAdvantages(dimensions).map(a => `
                        <div style="background: rgba(16,185,129,0.1); border-left: 3px solid #10b981; padding: 12px 14px; margin-bottom: 10px; border-radius: 0 8px 8px 0;">
        ...
        <div style="margin: 16px;">
            <div class="card" style="background: rgba(32,58,67,0.7); border-radius: 16px; overflow: hidden; border: 1px solid rgba(58,106,122,0.3);">
                ...
                <div style="padding: 20px;">
                    ${generateTeamRisks(dimensions).map(r => `
                        <div style="background: rgba(245,158,11,0.1); border-left: 3px solid #f59e0b; padding: 12px 14px; margin-bottom: 10px; border-radius: 0 8px 8px 0;">
    ...
    if (testedMembers.length < 2) {
        return `
            <div style="padding: 40px; text-align: center;">
                ...
                <div style="font-size: 16px; color: #fff; margin-bottom: 8px;">需要至少 2 位已测试成员</div>
                <div style="font-size: 13px; color: #86868b;">当前有 ${testedMembers.length} 位已测试成员</div>
            </div>
        `;
    }
    ...
    if (testedMembers.length < 3) {
        return `
            <div style="padding: 40px; text-align: center;">
                ...
                <div style="font-size: 16px; color: #fff; margin-bottom: 8px;">需要至少 3 位已测试成员</div>
                <div style="font-size: 13px; color: #86868b; margin-bottom: 20px;">当前有 ${testedMembers.length} 位已测试成员</div>
                <button onclick="addTeamMember()" style="background: linear-gradient(135deg, #2c5364, #d4af37); color: #0f2027; border: none; padding: 12px 32px; border-radius: 980px; font-size: 14px; font-weight: 600; cursor: pointer;">
                    ➕ 添加成员
                </button>
            </div>
        `;
            ...
            <div class="card" style="background: rgba(32,58,67,0.7); border-radius: 16px; overflow: hidden; border: 1px solid rgba(58,106,122,0.3);">
                ...
                <div style="padding: 20px;">
                    <div style="background: linear-gradient(135deg, #0f2027, #203a43, #2c5364); padding: 20px; border-radius: 12px; margin-bottom: 20px; border: 2px solid #d4af37;">
                        ...
                        <button onclick="unlockTeamReport()" style="width: 100%; background: #d4af37; color: #0f2027; border: none; padding: 14px; border-radius: 980px; font-size: 14px; font-weight: 600; cursor: pointer;">
                            立即解锁
                        </button>
                    </div>
...
function generateMemberComparison(m1, m2) {
    ...
    if (complementCount >= 3) {
        complementarity = '高度互补！你们在 ' + complementCount + ' 个维度上不同，可以形成很强的优势互补。建议在项目中分工合作，发挥各自特长。';
    } else if (complementCount === 2) {
        complementarity = '中度互补！你们在 ' + complementCount + ' 个维度上不同，有一定的互补性。注意沟通方式，可以相互学习。';
    } else if (complementCount === 1) {
        complementarity = '轻度互补！你们在 ' + complementCount + ' 个维度上不同，风格较为相似。合作默契度高，但需注意盲点重叠。';
    } else {
        complementarity = '风格相似！你们在所有维度上都一致，思维方式和行为模式非常接近。合作默契，但要注意避免相同盲点。';
    }
    ...
    if (!caution || caution === '合作时请注意：') {
        caution = '保持良好沟通，相互尊重，发挥各自优势。';
    }
    
    return { complementarity, strengths, caution };
}
...
function getStyleAnimal(code) {
    ...
    if (firstChar === 'A') return '🦉';
    if (firstChar === 'I') return '🦊';
    return '🦅';
}
...
function addTeamMember() {
    ...
    team.members.push({ 
        ...
        completed: true,
        styleName: getStyleName(code.toUpperCase())
    });
